...
function loadNotaBeneficiado(){
	...
	if($('id_notafiscal').value != ""){
		new Ajax.Request('./modulos/auditoria/lista_notas_fiscais.php', {
			method: 'post',
 ...
 */
function loadNotaFiscal(requisicao, empenho){
    ...
    if(globalWin != ""){ globalWin.destroy(); }
        ...
        width:largura,
        height:altura,
        title:"Análise da Nota Fiscal",
        url: "./modulos/auditoria/nova_analise_nota.php?id_requisicao="+requisicao+"&id_empenho="+empenho+"&id_usuario=<?php echo $sessao_usuario?>&nm_usuario=<?php echo $sessao_nome_usuario?>&nm_unidade=<?php echo $sessao_nome_unidade?>",
