...
//echo $data_i1." ".$row3["prospecto_ultimo_historico"]."<br>";
  ...
  <?php } ?>
	...
	<a href="javascript:popUP('minhaJanela1', 'historico_cli.php?id=<?php echo $row3["cliente_id"]; ?>', 'width=550,height=500,toolbar=no,menubar=no,status=no,scrollbars=no,resizable=yes')">Adicionar Observação</a><?php } ?></td>
    <td><?php  echo $row3['cliente_origem']; ?></td>
    <td><?php  echo $data_i1; ?></td>
    <td><a href="gera_pdf.php?id=<?php echo $row3["cliente_id"]; ?>" target="_blank"><img src="images/view.png" width="21" /></a><?php if($row3["cliente_standby"] == '1' ){ ?><a href="standby.php?id=<?php echo $row3["cliente_id"]; ?>"><img src="images/emoticon_sad.png" width="20" title="Não deseja ser mais ser cliente" border="0" /></a><?php }else{ ?><a href="voltar_cliente.php?id=<?php echo $row3["cliente_id"]; ?>"><img src="images/emoticon_smile.png" width="20" style="padding-right: 5px;" alt="Enviar Para Clientes" title="Quer voltar a ser cliente" border="0"/></a><?php } ?><?php if($nivel == "1"){ ?><a href="<?php $_SERVER['PHP_SELF']; ?>?id=<?php echo $row3["cliente_id"]; ?>&action=del" ><img src="images/icon_excluir.png" width="17" /></a><?php } ?></td>
