...
function actb(obj,evt,ca,cl,cal,tamanhox) {
  var actb_timeOut=-1000; // Autocomplete Timeout in ms (-1: autocomplete never time out)
  var actb_lim=20;    // Number of elements autocomplete can show (-1: no limit)
  ...
  var actb_bgColor='#EAEAEA';
  var actb_textColor='#000000';  //#227A22
  var actb_border='#89AA69 1px solid'; 
  var actb_hColor='#FFFFFF';
  var actb_fFamily='Verdana';
  var actb_fSize='11px';
  var actb_hStyle='color: #FF0000;';
  var actb_delimwords=new Array();
  var actb_cdelimword=0;
  var actb_delimchar=new Array();
  var actb_keywords=new Array();
  var actb_keylinks=new Array();
  ...
  function actb_parse(n) {
    if (actb_delimiter.length > 0) { var t=actb_delimwords[actb_cdelimword].trim().addslashes(); var plen=actb_delimwords[actb_cdelimword].trim().length; }
    else { var t=actb_curr.value.addslashes(); var plen=actb_curr.value.length; }
    var tobuild=''; var i;
    if (actb_firstText) { var re=new RegExp("^" + t, "i"); }
  ...
  function curTop() {
    actb_toreturn=0;
    obj=actb_curr;
    while(obj) { actb_toreturn+=obj.offsetTop; obj=obj.offsetParent; }
    return actb_toreturn;
  }
  function curLeft() {
    actb_toreturn=0;
    obj=actb_curr;
    while(obj) { actb_toreturn+=obj.offsetLeft; obj=obj.offsetParent; }
    return actb_toreturn;
  }
  function actb_generate() {
    ...
    a.cellSpacing='1px';
    a.cellPadding='2px';
    a.width=50; // Tamanho minimo da tabela
    if (tamanhox!='') { a.width=tamanhox; }
    a.style.position='absolute';
    a.style.top=eval(curTop() + actb_curr.offsetHeight) + "px";
    a.style.left=curLeft() + "px";
    a.style.backgroundColor=actb_bgColor;
    a.style.border=actb_border;
    a.id='tat_table';
    document.body.appendChild(a);
    var i;
    var first=true;
    var j=1;
    if (actb_mouse) { a.onmouseout= actb_table_unfocus; a.onmouseover=actb_table_focus; }
    var counter=0;
    for (i=0;i<actb_keywords.length;i++) {
      if (actb_bool[i]) {
        ...
        if (first && !actb_tomake) {
          ...
          first=false;
          actb_pos=counter;
        } else { r.style.backgroundColor=actb_bgColor; }
        r.id='tat_tr'+(j);
        c=r.insertCell(-1);
        ...
        if (actb_mouse) { c.onclick=actb_mouseclick; c.onmouseover=actb_table_highlight; }
        j++;
      }
      if (j - 1 == actb_lim && j < actb_total) {
        ...
        c.style.fontSize=actb_fSize;
        c.align='center';
        if (actb_mouse) { c.onclick=actb_mouse_down; }
        break;
      }
    }
    actb_rangeu=1;
  ...
  function actb_remake() {
    ...
    if (actb_rangeu > 1) {
      ...
      c.style.fontSize=actb_fSize;
      c.align='center';
      if (actb_mouse) { c.onclick=actb_mouse_up; }
    }
    for (i=0;i<actb_keywords.length;i++) {
      if (actb_bool[i]) {
  ...
  function actb_insertword(a) {
    if (actb_delimiter.length > 0) {
      ...
      for (i=0;i<actb_delimwords.length;i++) {
        if (actb_cdelimword == i) { str += a; l=str.length; }
        else { str += actb_delimwords[i]; }
        if (i != actb_delimwords.length - 1) { str += actb_delimchar[i]; }
      }
      actb_curr.value=str;
      setCaret(actb_curr,l);
      /*
      objt_atual=actb_curr.name; 
      objt_destino=objt_atual.replace('cme','hdn'); // AQUI
    */
    } else { actb_curr.value=a; }
    actb_mouse_on_list=0;
    actb_removedisp();
  }
  function actb_penter() {
    ...
    for (var i=0;i<=actb_keywords.length;i++) {
      if (actb_bool[i]) c++;
      if (c == actb_pos) { word=actb_keywords[i]; lidx=i; break; }
    }
    actb_insertword(word);
    campo_destino.value=actb_keylinks[lidx];
    if (campo_blur!="") {
      eval(campo_blur);
    }
    //document.form[objt_destino].value=actb_keylinks[lidx]; // AQUI
    //alert(objt_destino.indexOf('L'));
    /*
    if (objt_destino.indexOf('L')<0) {
      comblur=objt_destino+'blur();';
      eval(comblur);
    }
    */
  }
  ...
  function actb_tocomplete(kc) {
    ...
    if (actb_display) {
      ...
      for (var i=0;i<=actb_keywords.length;i++) {
        if (actb_bool[i]) c++;
        if (c == actb_pos) { word=i; break; }
      }
      actb_pre=word;
    } else { actb_pre=-1};
    if (actb_curr.value == '') { actb_mouse_on_list=0; actb_removedisp(); return; }
    if (actb_delimiter.length > 0) {
      caret_pos_start=getCaretStart(actb_curr);
      caret_pos_end=getCaretEnd(actb_curr);
      delim_split='';
      for (i=0;i<actb_delimiter.length;i++) { delim_split += actb_delimiter[i]; }
      ...
      for (i=0,j=actb_curr.value.length;i<actb_curr.value.length;i++,j--) {
        if (actb_curr.value.substr(i,j).search(delim_split_rx) == 0) {
          ...
          c++;
          actb_delimwords[c]='';
        } else { actb_delimwords[c] += actb_curr.value.charAt(i); }
      }
      var l=0;
      actb_cdelimword=-1;
      for (i=0;i<actb_delimwords.length;i++) {
        if (caret_pos_end >= l && caret_pos_end <= l + actb_delimwords[i].length) { actb_cdelimword=i; }
        l+=actb_delimwords[i].length + 1;
      }
      var t=actb_delimwords[actb_cdelimword].addslashes().trim();
    } else { var t=actb_curr.value.addslashes(); }
    if (actb_firstText) { var re=new RegExp("^" + t, "i"); }
    else { var re=new RegExp(t, "i"); }
    actb_total=0;
    actb_tomake=false;
